...
def image_preprocessing(img, width, height, gray_para, blur_para, show_name, d):
	...
	resized_img = resizeimage.resize_thumbnail(img, [width, height])
	width, height = resized_img.size
	ratio_img = width / height
	# convert the resized image to grayscale, blur it slightly,
	# and threshold it
	gray_img = ImageOps.grayscale(resized_img)
	blurred_img = gray_img.filter(ImageFilter.GaussianBlur(radius = 5))
	thresh_img = blurred_img.point(lambda p: p > blur_para[0] and blur_para[1])
...
def thresh_img_preprocessing(img, width, gray_para, blur_para, show_name, d):
	resized_img = imutils.resize(img, width)
	ratio_img = resized_img.shape[0] / float(resized_img.shape[0])
...
def main(Cimg, d):
	...
	if len(cnt_Cimgs) == 0:
		...
		if len(cnt_Cimgs) == 1:
			...
			cnts_rotated_img = findContours(thresh_rotated_img)
			cnt_rotated_img = find_max_contour(cnts_rotated_img, ratio_rotated_img, resized_rotated_img, d)
	...
	resized_cropped_img, ratio_cropped_img, gray_cropped_img, blurred_cropped_img, thresh_cropped_img = thresh_img_preprocessing(cropped_img, w_cropped_img, cropped_img_gray_para_rect, cropped_img_blur_para_rect, 'cropped_img', d)
	...
	##**************** Getting Mode of reference rect color*******************

	# Processing the cropped_img.
	h_cropped_img, w_cropped_img, c_cropped_img = cropped_img.shape
	cropped_img_gray_para = [5, 5, 1]
	cropped_img_blur_para = [185, 255]
	resized_cropped_img, ratio_cropped_img, gray_cropped_img, blurred_cropped_img, thresh_cropped_img = image_preprocessing(cropped_img, w_cropped_img, cropped_img_gray_para, cropped_img_blur_para, 'cropped_img_reference', d)
	cnts_color_rect = findContours(thresh_cropped_img)
...
if __name__ == '__main__':
	...
	# Cimg = cv2.imread("R_C_img/test2.jpg")
	Cimg = Image.open(r'R_C_img/test2.jpg')
