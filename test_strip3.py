...
def main(Cimg, d):
	try:
		...
		if len(cnt_Cimgs) == 0:
			...
			if len(cnt_Cimgs) == 1:
				...
				if valid == "yes":
					...
					return "There is a suitable contour but not any suitable cropped images."
			else:
				# print("There are too many contours.")		
				# return "Please retry on black background."
				return "There are too many contours."
		...
		rect_min_max_area_ratio = [8, 350]
		rect_min_max_wh_ratio = [0.5, 2]
		...
		if len(cnt_candidates) < 4:
			# print("There aren't enough contours.")
			# return "Please retry on black background."
			return "There aren't enough contours."
		...
		if valid_refer == "yes":
			...
			# return "Please retry on black background."
			return "There isn't a suitable refernce rect."
		...
		index = [first_index + 1, second_index + 1, third_index + 1, fourth_index + 1]
		print(index)
		...
		return test_strip_mean_color, index
	except:
...
if __name__ == '__main__':
	...
	Cimg = cv2.imread("R_C_img/test6.jpg")
